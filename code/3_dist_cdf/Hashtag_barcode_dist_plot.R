...
barcode_dist_plot <- function(mtx){
  ...
  # the cell with highest Hashtags
  ...
  barplot(target_dist, las=2, main=c(paste0("The cell with highest overall Hashtags"),paste0("(barcode: ",max_barcode, ")")))
  title(xlab="Target (Hashtag)", line=3)
  title(ylab="UMI counts", line=3.1)
  dev.off()

  # the cell with lowest overall Hashtags
  target_dist <- mtx[,min_barcode]
  names(target_dist) <- 1:length(target_dist)
  filename <- paste0("./analysis/cache/2_", deparse(substitute(mtx)), "_min_barcode.tif")
  tiff(file=filename, width=8, height=5, units="in", res=600)
  barplot(target_dist, las=2, main=c(paste0("The cell with lowest overall Hashtags"),paste0("(barcode: ",min_barcode,")")))
  title(xlab="Target (Hashtag)", line=3)
  title(ylab="UMI counts", line=3.1)
  dev.off()
