...
target_dist_plot <- function(mtx){
  ...
  barplot(barcode_dist, las=2, main=c(paste0("The highest gRNA in all cells"),paste0("(targeting: ",max_target,")")))
  ...
  tiff(file=filename, width=8, height=5, units="in", res=600)
  barplot(barcode_dist, las=2, main=c(paste0("The lowest gRNA in all cells"),paste0("(targeting: ",min_target,")")))
  title(xlab="Barcode (Cell)", line=3.7)
  title(ylab="UMI counts", line=3.1)
  dev.off()
  ...
  tiff(file=filename, width=8, height=5, units="in", res=600)
  barplot(target_dist, las=2, main=c(paste0("Non-zero UMI counts for each target (gRNA)"),paste0("(non-cumulative)")))
  title(xlab="Target (gRNA)", line=3)
  ...
  tiff(file=filename, width=8, height=5, units="in", res=600)
  plot(ecdf(target_dist), main="CDF: UMI counts for each target (gRNA)", xlab="UMI counts for each target (gRNA)", ylab="CDF")
  dev.off()

  # UMI counts for all cells (pdf)
  filename <- paste0("./analysis/cache/10_", deparse(substitute(mtx)), "_target_dist_pdf.tif")
  tiff(file=filename, width=8, height=5, units="in", res=600)
  PDF <- hist(target_dist, freq=F, breaks=200, main="PDF: UMI counts in each barcode (cell)", xlab="UMI counts in each barcode (cell)", ylab="PDF")
  dev.off()
