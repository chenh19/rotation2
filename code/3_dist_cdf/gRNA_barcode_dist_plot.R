...
barcode_dist_plot <- function(mtx){
  ...
  barplot(target_dist, las=2, main=c(paste0("The cell with highest overall gRNA expression"),paste0("(barcode: ",max_barcode, ")")))
  ...
  tiff(file=filename, width=8, height=5, units="in", res=600)
  barplot(target_dist, las=2, main=c(paste0("The cell with lowest overall gRNA expression"),paste0("(barcode: ",min_barcode,")")))
  title(xlab="Target (gRNA)", line=3)
  title(ylab="UMI counts", line=3.1)
  dev.off()
  ...
  tiff(file=filename, width=8, height=5, units="in", res=600)
  PDF <- hist(barcode_dist, freq=F, breaks=100, main="PDF: UMI counts in each barcode (cell)", xlab="UMI counts in each barcode (cell)", ylab="PDF")
  dev.off()
