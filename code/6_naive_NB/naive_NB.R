...
nbregression <- function(sample){
  ...
  for (i in 1:count){
    ...
    pair <- data.frame(gene_data, gRNA_data)
    test <- try(glm.nb(gene_data ~ gRNA_data, data = pair, start = NULL))
    genes <- c(genes, gene)
    gRNAs <- c(gRNAs, gRNA)
  ...
  filename <- paste0("./output/SCEPTRE/", deparse(substitute(sample)), "_pvalues.csv")
...
negative_control <- read.csv("./output/SCEPTRE/negative_control_pvalues.csv")
negative_control <- filter(negative_control, !is.na(p_value))
positive_control <- read.csv("./output/SCEPTRE/positive_control_pvalues.csv")
